...
async def get_position(pg_width, pg_height, text_width, position):
    bottomLeft = {
        ...
        "M": [int((pg_width - text_width) / 2), int((pg_height - pg_height / 10) / 2)],
        "B": [int((pg_width - text_width) / 2), int(pg_height - pg_height / 10)],
    }
        ...
        with fitz.open(input_file) as pdf:
            for page in pdf:
                ...
                text_width = font.text_length(
                    watermark_text, fontsize=int(page.bound().height // 5)
                )

                tw = fitz.TextWriter(
                    page.rect, opacity=int(opacity) / 5, color=COLOR_CODE
                )
                ...
                tw.append(
                    (txt_bottom, txt_left),
                    watermark_text,
                    fontsize=int(page.bound().height // 5),
                    font=font,
                )
                tw.write_text(page)
